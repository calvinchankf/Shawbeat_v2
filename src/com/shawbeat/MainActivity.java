...
import com.database.EditionDBHelper;
...
public class MainActivity extends ActionBarActivity implements DownloadEditionCompleteListener{
    ...
    private List<Edition> mEdition = new ArrayList<Edition>();
    private ArrayList<String> mDrawerTitles = new ArrayList<String>();
    private boolean firstLaunch = true;
    private EditionDBHelper editionDBHelper = new EditionDBHelper(MainActivity.this);
    ...
    private void selectItem(int position) {
    	...
    	ContentFrameFragment fragment = new ContentFrameFragment();
    	
    	Bundle args = new Bundle();
    	args.putInt("ed_id", Integer.valueOf(mEdition.get(position).getEdition_ID()));
    	fragment.setArguments(args);
    	
    	getSupportFragmentManager().beginTransaction().add(R.id.content_frame, fragment).commit();
    	
    	setTitle(mDrawerTitles.get(position));
    	mDrawerLayout.closeDrawer(mRefreshDrawerList);
    }
	...
	@Override
	public void downloadEditionResultCallback(List<Edition> s) {
		if (s!=null){
			mEdition = s;
			
			if (mDrawerTitles.size()>0) mDrawerTitles.clear();
			for (Edition e : s){
				mDrawerTitles.add(e.getEdition_name());
			}
			mAdapter.notifyDataSetChanged();
			
			editionDBHelper.insertEdition(s);
			if (firstLaunch){
				selectItem(0);
